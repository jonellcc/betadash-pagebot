...
module.exports = {
  name: 'nightmotion',
  description: 'nightmotion canvas',
  usage: 'nightmotion <userid>',
  author: 'cliff',
  async execute(senderId, args, pageAccessToken, sendMessage) {
    ...
    try {    
      const apiUrl = `https://api-canvass.vercel.app/night-motion?userid=${encodeURIComponent(uid)}`;

const nya = await axios.get(apiUrl);
const res = nya.data.imageUrl;

      await sendMessage(senderId, { attachment: { type: 'gif', payload: { url: res } } }, pageAccessToken);
